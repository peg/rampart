...
| OpenClaw >= 2026.4.29 | Native plugin (`rampart setup openclaw`) | Yes | OpenClaw first-class plugin approvals / native approval UI | Preferred RC baseline: explicit startup activation, all tool calls intercepted by the plugin, degraded behavior depends on tool class and `failOpenTools` | **Recommended** |
| OpenClaw 2026.4.11 - 2026.4.28 | Native plugin (`rampart setup openclaw`) | Yes | OpenClaw native approval UI | All tool calls are intercepted by the plugin; degraded behavior depends on tool class and `failOpenTools` | **Supported** |
