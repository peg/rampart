...
**Native plugin (OpenClaw >= 2026.3.28):** Installs the bundled `before_tool_call` hook plugin. Intercepts all tool calls (exec, read, write, web_fetch, browser, message); degraded behavior depends on tool class and configuration. Plugin is embedded in the `rampart` binary — no external download required.
