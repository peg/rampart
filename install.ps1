...
if ($TestMode) {
    ...
    if (-not (Test-StrictReleaseVersion $expectedCandidateVersion)) {
        Write-Err "Invalid release version: $expectedCandidateVersion. Expected a tag such as v1.4.0 or v1.4.0-rc.1."
        exit 1
    }
    ...
    if (-not (Test-StrictReleaseVersion $version)) {
        Write-Err "Invalid release version: $version. Expected a tag such as v1.4.0 or v1.4.0-rc.1."
        exit 1
    }
