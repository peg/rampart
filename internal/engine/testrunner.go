...
type TestResult struct {
	...
	// Passed is true if the result matched expectations.
	Passed bool

	// Decision is the engine's actual decision.
	Decision Decision

	// ExpectedAction is what was expected.
	ExpectedAction Action

	// Error is set if the test case itself was invalid.
	Error error
}
...
// LoadInlineTests attempts to extract inline tests from a policy file.
...
func LoadInlineTests(policyPath string) (*TestSuite, error) {
	...
	// Parse into a generic map to check for tests key.
	...
	// Re-marshal just the tests portion and unmarshal into []TestCase.
...
func runSingleTest(eng *Engine, tc TestCase) TestResult {
	...
	if err != nil {
		return TestResult{
			Case:  tc,
			Error: fmt.Errorf("invalid expect value %q: %w", tc.Expect, err),
		}
	}
	...
	if tc.Tool == "" {
		return TestResult{
			Case:  tc,
			Error: fmt.Errorf("test case %q: tool is required", tc.Name),
		}
	}
	...
	// Check message glob if specified.
