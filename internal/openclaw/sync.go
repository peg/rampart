...
// Package openclaw provides integration helpers for OpenClaw. The legacy sync
// helper is an advisory migration reference only; OpenClaw configuration
// cannot represent Rampart policies losslessly.
package openclaw
...
func FormatOpenClawConfig(result *SyncResult) string {
	...
	fmt.Fprintf(&b, "// ADVISORY ONLY: not equivalent Rampart enforcement\n")
	fmt.Fprintf(&b, "// Use `rampart protect openclaw` for the supported native plugin boundary.\n")
	fmt.Fprintf(&b, "// Generated by: rampart openclaw sync (deprecated)\n")
	fmt.Fprintf(&b, "// Source: %s (%s profile)\n", result.SourceFile, result.Profile)
	fmt.Fprintf(&b, "// Generated: %s\n", result.GeneratedAt.Format(time.RFC3339))
	fmt.Fprintf(&b, "//\n")
	fmt.Fprintf(&b, "// Review manually; do not treat this output as a complete security policy.\n")
	if len(result.ReadDenyPaths) > 0 || len(result.WriteDenyPaths) > 0 {
		fmt.Fprintf(&b, "// File read/write restrictions are not translated because OpenClaw cannot represent them equivalently.\n")
	}
	fmt.Fprintf(&b, "\n")
...
func buildExecApprovals(result *SyncResult) map[string]any {
	...
	return map[string]any{
		...
		"_generated_by": fmt.Sprintf("advisory rampart openclaw sync from %s", result.SourceFile),
	}
}
