...
func TestBuildExactAllowPattern(t *testing.T) {
	tests := []struct {
		...
		{"sudo docker run nginx", "sudo docker run nginx"},
		{"sudo kubectl apply -f prod.yaml", "sudo kubectl apply -f prod.yaml"},
		{"chmod 777 /tmp/../etc/shadow", "chmod 777 /tmp/../etc/shadow"},
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			got := BuildExactAllowPattern(tt.input)
